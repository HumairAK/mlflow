...
def test_get_workspace_returns_entity(store, host_creds):
    ...
    response.workspace.trace_archival_location = "s3://archive/team-b"
    response.workspace.trace_archival_retention = "30d"
    with mock.patch(
        "mlflow.store.workspace.rest_store.call_endpoint", return_value=response
    ) as call_endpoint:
...
def test_create_workspace_sends_payload(store, host_creds):
    ...
    response.workspace.trace_archival_location = "s3://archive/team-c"
    response.workspace.trace_archival_retention = "14d"
    with mock.patch(
        "mlflow.store.workspace.rest_store.call_endpoint", return_value=response
    ) as call_endpoint:
...
def test_update_workspace_returns_new_description(store, host_creds):
    ...
    response.workspace.trace_archival_location = "s3://archive/team-e"
    response.workspace.trace_archival_retention = "7d"
    with mock.patch(
        "mlflow.store.workspace.rest_store.call_endpoint", return_value=response
    ) as call_endpoint:
