...
def _parse_trace_archival_duration_millis(value: str | None) -> int | None:
    ...
    match = _TRACE_ARCHIVAL_DURATION_PATTERN.fullmatch(value.strip())
    if match is None:
...
def _parse_archive_now_request(value: str | None) -> _ArchiveNowRequest | None:
    if value is None:
        return None
    ...
    try:
        ...
        _logger.warning("Ignoring malformed trace archive-now tag value: %r", value)
        return None

    if not isinstance(payload, dict):
        _logger.warning("Ignoring non-object trace archive-now tag value: %r", value)
        return None

    older_than = payload.get("older_than")
    if older_than is None:
        return _ArchiveNowRequest(older_than_millis=None)

    try:
        return _ArchiveNowRequest(older_than_millis=_parse_trace_archival_duration_millis(older_than))
    except MlflowException:
        _logger.warning("Ignoring malformed trace archive-now older_than value: %r", older_than)
        return None
...
def _parse_experiment_trace_archival_retention_millis(value: str | None) -> int | None:
    ...
    try:
        ...
        _logger.warning("Ignoring malformed trace archival retention tag value: %r", value)
        return None

    if not isinstance(payload, dict) or payload.get("type") != "duration":
        _logger.warning("Ignoring unsupported trace archival retention tag value: %r", value)
        return None

    try:
        return _parse_trace_archival_duration_millis(payload.get("value"))
    except MlflowException:
        _logger.warning("Ignoring malformed trace archival retention duration: %r", value)
        return None
...
class SqlAlchemyStore(SqlAlchemyGatewayStoreMixin, AbstractStore):
    ...
    def archive_traces(
        ...
        *,
        ...
        long_retention_allowlist = {str(experiment_id) for experiment_id in long_retention_allowlist or []}
        broader_retention_millis = _parse_trace_archival_duration_millis(
            self._resolve_workspace_trace_archival_retention(default_retention)
        )
        ...
        with self.ManagedSessionMaker() as session:
            experiments = (
                self
                ._get_query(session, SqlExperiment)
                .options(joinedload(SqlExperiment.tags))
                .filter(SqlExperiment.lifecycle_stage == LifecycleStage.ACTIVE)
                .all()
            )

            emergency_requests: list[tuple[str, _ArchiveNowRequest]] = []
            regular_cutoffs: list[tuple[str, int]] = []
            for experiment in experiments:
                experiment_id = str(experiment.experiment_id)
                experiment_tags = {tag.key: tag.value for tag in experiment.tags}

                if archive_now := _parse_archive_now_request(
                    experiment_tags.get(TraceExperimentTagKey.ARCHIVE_NOW)
                ):
                    emergency_requests.append((experiment_id, archive_now))

                if retention_millis := self._resolve_effective_trace_archival_retention_millis(
                    experiment_id=experiment_id,
                    experiment_tags=experiment_tags,
                    broader_retention_millis=broader_retention_millis,
                    long_retention_allowlist=long_retention_allowlist,
                ):
                    regular_cutoffs.append((experiment_id, now_millis - retention_millis))

            emergency_candidates: list[_TraceArchiveCandidate] = []
            for experiment_id, archive_now in emergency_requests:
                older_than_cutoff = (
                ...
                emergency_candidates.extend(
                    self._find_archivable_trace_candidates(
                        session=session,
                        experiment_id=experiment_id,
            ...
            if len(emergency_candidates) < max_traces:
                for experiment_id, cutoff in regular_cutoffs:
        ...
        candidates = self._dedupe_trace_archive_candidates(emergency_candidates, regular_candidates)
        archived_count = 0
        for candidate in candidates[:max_traces]:
    ...
    def _resolve_trace_archival_root(
        ...
        return trace_archival_location, False
    ...
    def _resolve_effective_trace_archival_retention_millis(
        ...
        *,
        ...
        broader_retention_millis: int | None,
    ...
    def _dedupe_trace_archive_candidates(
        ...
        for candidates in (
            sorted(emergency_candidates, key=lambda c: (c.timestamp_ms, c.trace_id)),
    ...
    def _find_archivable_trace_candidates(
        self,
        *,
        session: Session,
        experiment_id: str,
        ...
        statement = (
            self
            ._trace_query(session)
            .with_entities(SqlTraceInfo.request_id, SqlTraceInfo.timestamp_ms)
            .filter(
        ...
        return [
            ...
            for trace_id, timestamp_ms in statement.all()
        ]
    ...
    def _deserialize_trace_archival_snapshot(
    ...
    def _get_archival_repository_artifact_uri(
        ...
        *,
        ...
        resolved_root, append_workspace_prefix = self._resolve_trace_archival_root(
            trace_archival_location
        )
        if resolved_root is None:
            return get_artifact_uri_for_trace(trace_info)

        if append_workspace_prefix and (workspace_name := self._get_trace_archival_workspace_name()):
    ...
    def _finalize_archived_trace(
        ...
        *,
        ...
        with self.ManagedSessionMaker() as session:
            ...
            current_snapshot_rows = sorted((span.span_id, span.content) for span in sql_trace_info.spans)
            if current_snapshot_rows != snapshot_rows:
                _logger.info(
                    "Skipping archival finalization for trace %s because its DB-backed spans changed.",
                    trace_id,
                )
                return False
    ...
    def _mark_trace_archival_failure(self, *, trace_id: str, failure_reason: str) -> None:
        with self.ManagedSessionMaker() as session:
            ...
            trace_info = sql_trace_info.to_mlflow_entity()
            if trace_info.tags.get(TraceTagKey.SPANS_LOCATION) not in (
                None,
        ...
        *,
        ...
        with self.ManagedSessionMaker() as session:
            for experiment_id, archive_now in emergency_requests:
                ...
                if self._find_archivable_trace_candidates(
                    session=session,
                    experiment_id=experiment_id,
                    max_timestamp_millis=older_than_cutoff,
                    limit=1,
                ...
                (
                    ...
                    .filter(
                        SqlExperimentTag.experiment_id == int(experiment_id),
